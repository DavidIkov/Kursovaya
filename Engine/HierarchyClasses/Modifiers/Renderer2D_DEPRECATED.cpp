...
void Renderer2D::TypesOfObjectsRendering_Element::Bind(Object2DData* instPtr, std::vector<float>* vertexBuffer, std::vector<unsigned int>* indexBuffer) {
...
void Renderer2D::Render(int width, int height) {
	...
	Camera2D* cam = instancePtr->FindTypePtr<Camera2D>(Camera2D::TypeIndex);

	struct Cords {
		Object2DData* InstancePtr;
		float PosX;
		float PosY;
		float SizeX;
		float SizeY;
	...
	for (unsigned int i = 0; i < FilteredOrder.size(); i++) {
		...
		Object2DData* Element = FilteredOrder[i];
